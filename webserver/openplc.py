    ...
    def __init__(self, stream):
        ...
        def _populateQueue(stream, queue):
            ...
            while (self.end_of_stream == False):
                ...
                if line:
                    queue.put(line)
                    if (line.find("Compilation finished with errors!") >= 0 or line.find(
                            "Compilation finished successfully!") >= 0):
...
class runtime:
        ...
        if (self.status() == "Running"):
            try:
                ...
                while self.theprocess.poll() is None:  # XXX: iPAS, to prevent the defunct killed process.
                    time.sleep(
                        1)  # https://www.reddit.com/r/learnpython/comments/776r96/defunct_python_process_when_using_subprocesspopen/
    ...
    # TODO: clean this up
    def start_opcua_server(self, port_num):
        if (self.status() == "Running"):
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s.connect(('localhost', 43628))
                s.send(b'start_opcuaserver(' + (b'%d' % port_num) + b')\n')
                data = s.recv(1000)
                s.close()
            except:
                print("Error connecting to OpenPLC runtime")
